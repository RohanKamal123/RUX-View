...
def sample_event_data(sample_user_id, sample_location_id, sample_camera_id) -> dict:
    return {
        ...
        "incident_id": "INC_001",
        "timestamp_start": datetime(2026, 4, 25, 10, 0, 0),
        "timestamp_end": datetime(2026, 4, 25, 10, 5, 30),
        "duration_sec": 330.0,
        "threat_level": "MEDIUM",
        "alert_sent": False,
        "alert_type": "LOG",
...
async def test_insert_person(
    ...
    person = await create_person(
        db_session,
        {
            ...
            "user_id": sample_user_id,
            "location_id": sample_location_id,
            "first_seen": datetime(2026, 4, 25, 10, 0, 0),
            "last_seen": datetime(2026, 4, 25, 10, 30, 0),
            "sighting_count": 1,
            "threat_flags": 0,
            "is_staff": False,
            "user_label": "Unknown",
...
async def test_update_person_sighting(
    ...
    await create_person(
        db_session,
        {
            ...
            "location_id": sample_location_id,
            "sighting_count": 5,
            "first_seen": datetime(2026, 4, 25, 8, 0, 0),
        },
    )
    ...
    for i in range(3):
        await create_sighting(
            db_session,
            {
                ...
                "user_id": sample_user_id,
                "camera_id": "CAM_001",
                "timestamp": datetime(2026, 4, 25, 10 + i, 0, 0),
            },
        )
    ...
    for hour in [8, 10, 12]:
        await save_scene_state(
            db_session,
            {
                "camera_id": sample_camera_id,
                "user_id": sample_user_id,
                "timestamp": datetime(2026, 4, 25, hour, 0, 0),
                "person_count": hour,
            },
        )
...
async def test_insert_audio_event(db_session: AsyncSession, sample_user_id):
    ...
    audio = await create_audio_event(
        db_session,
        {
            ...
            "threat_level": "HIGH",
            "expires_at": datetime(2026, 4, 28, 10, 0, 0),
        },
    )
...
async def test_get_expired_transcripts(db_session: AsyncSession, sample_user_id):
    ...
    await create_audio_event(
        db_session,
        {
            ...
            "expires_at": datetime(2026, 4, 22, 10, 0, 0),
        },
    )
    # Create a non-expired transcript
    await create_audio_event(
        db_session,
        {
            "camera_id": "CAM_002",
