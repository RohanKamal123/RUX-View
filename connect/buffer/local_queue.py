...
class LocalQueue:
    ...
    def enqueue(self, trigger_data: dict) -> None:
        ...
        with self._lock:
            if self._conn is None:
                logger.warning("Queue is closed — cannot enqueue event")
                return
    ...
    def dequeue_all(self) -> list[dict]:
        ...
        with self._lock:
            if self._conn is None:
                logger.warning("Queue is closed — cannot dequeue events")
                return []
        ...
        for event in events:
            try:
                ...
                if result.success:
                    ...
                    # Re-enqueue failed events (if queue is still open)
                    self.enqueue(event)

            except Exception:
                logger.exception("Failed to flush event to server")
                # Only re-enqueue if the queue is still open
