...
- Audio analysis (transcription + threat detection)
...
# ── New unified Gemini response parser ─────────────────────────


def parse_gemini_response(raw_text: str) -> dict:
    """Parse a Gemini response that follows the standardised JSON format.

    Handles markdown fences, raw JSON, and graceful fallback on parse failure.
    Recognises the short-circuit ``{"change_detected": false}`` signal.

    Returns:
        dict with keys: change_detected, threat_level, description, action,
                        objects_detected
        OR ``{"change_detected": False}`` when the scene is unchanged.
    """
    try:
        ...
        if not result.get("change_detected", True):
...
# ── CHANGE_DETECTED APPEND TEXT ────────────────────────────────

_CHANGE_DETECTED_SUFFIX = """
...
If the scene is completely unchanged from normal activity,
respond ONLY with:
...
VALID_THREAT_LEVELS = {"LOW", "MEDIUM", "HIGH", "EMERGENCY", "CRITICAL"}


# ── Audio Analysis ─────────────────────────────────────────────

AUDIO_ANALYSIS_PROMPT = """Analyse this audio clip from a CCTV security system. 
Transcribe any speech and detect security threats.
Return ONLY valid JSON with EXACTLY these keys. No explanation. No markdown.

{
  "transcript": "transcribed text or empty string if no speech",
  "language": "bn|en|hi|ar|unknown",
  "threat_detected": true/false,
  "threat_level": "LOW|MEDIUM|HIGH|EMERGENCY",
  "threat_types": ["scream", "glass_breaking", "shouting", "gunshot", "explosion", "alarm", "struggle", "other"],
  "description": "brief description of what is heard",
  "confidence": 0.0-1.0
}

threat_types should include any detected threats from the controlled vocabulary.
Set threat_detected to true if any threat types are detected.
confidence represents overall certainty of the analysis.

If the audio is completely silent or contains no speech or threats,
respond ONLY with:
{"change_detected": false}"""
...
AUDIO_ANALYSIS_FALLBACK = {
    ...
    "threat_level": "LOW",
    "threat_types": [],
    "description": "Audio analysis unavailable",
    "confidence": 0.0,
}


async def analyse_audio(audio_bytes: bytes, mime_type: str = "audio/wav") -> dict:
    """Analyse audio using Gemini 2.5 Flash for transcription and threat detection.
    ...
    Returns:
        dict with keys: transcript, language, threat_detected, threat_level,
    ...
    try:
        ...
        if not all(k in result for k in required_keys):
            ...
            logger.warning("Audio analysis missing keys: %s", missing)
            # Fill in missing keys from fallback
            for key in missing:
                result[key] = AUDIO_ANALYSIS_FALLBACK[key]
        ...
        logger.error("analyse_audio failed: %s", exc, exc_info=True)
        return dict(AUDIO_ANALYSIS_FALLBACK)
...
async def analyse_frame_with_second_pass(jpeg_bytes: bytes) -> dict:
    ...
    try:
        ...
        # Check for no-change signal from Layer 1
        if not vision_result.get("change_detected", True):
...
async def analyse_frame(jpeg_bytes: bytes) -> dict:
    ...
    Returns:
        dict with keys: persons, person_count, scene_alerts, vehicles, gates_visible, change_detected
    """
    fallback = {
        "persons": [],
        "person_count": 0,
...
async def make_incident_decision(timeline: list, context: dict) -> dict:
    ...
    try:
        ...
        # Build camera context if available (Step 4)
        camera_context = context.get("camera_context", "")
        prompt_body = INCIDENT_DECISION_PROMPT.format(
