...
TIMING = {
    "indoor":  {"cooldown": 15, "burst": 2.5, "urgent": 0.8, "no_motion": 3, "max_cap": 60, "close_timeout": 15},
    "outdoor": {"cooldown": 30, "burst": 2.5, "urgent": 0.8, "no_motion": 6, "max_cap": 60, "close_timeout": 15},
    "parking": {"cooldown": 20, "burst": 2.5, "urgent": 0.8, "no_motion": 4, "max_cap": 120, "close_timeout": 15},
    "shop":    {"cooldown": 10, "burst": 2.5, "urgent": 0.8, "no_motion": 3, "max_cap": 300, "close_timeout": 15},
    "night":   {"cooldown": 8,  "burst": 1.5, "urgent": 0.8, "no_motion": 4, "max_cap": 60, "close_timeout": 15},
}
...
class IncidentTracker:
    ...
    def _process_tracking(self, trigger: TriggerData) -> IncidentAction:
        ...
        # 1b. Check close_timeout — if no new motion frames arrived within
        #     close_timeout seconds, force close and trigger final Gemini decision.
        #     This handles the case where triggers stop arriving entirely.
        if no_motion_sec >= timing["close_timeout"]:
            logger.info(
                "Incident %s: no motion for %.1fs (close_timeout=%ds) — force closing",
                self._state.incident_id, no_motion_sec, timing["close_timeout"],
            )
            return IncidentAction.CLOSE_INCIDENT
