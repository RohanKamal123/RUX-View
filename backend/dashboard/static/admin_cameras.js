/**
 * admin_cameras.js — Camera Management Dashboard UI
 * 
 * Features:
...
(function () {
  ...
  // ── API Calls (using working /api/cameras endpoints) ──────
  const API = {
    ...
    async getCameraHealth(cameraId, days = 7) {
      try {
        const details = await this.getCameraDetails(cameraId);
        return {
          snapshots: [],
          health: {
            status: details.status || 'unknown',
            last_heartbeat: details.last_seen_at || null,
            uptime_pct_24h: details.is_active ? 100 : 0,
            uptime_pct_7d: details.is_active ? 100 : 0,
            error_count_24h: 0,
            error_count_7d: 0,
          }
        };
      } catch {
        return { snapshots: [], health: { status: 'unknown' } };
      }
    },
    ...
    async getHealthSummary() {
      try {
        const data = await this.listCameras(1, 1000);
        ...
        return {
          ...
          error_cameras: 0,
          avg_uptime_pct: total > 0 ? (online / total) * 100 : 100,
        };
      } catch {
        return { total_cameras: 0, online_cameras: 0, offline_cameras: 0, error_cameras: 0, avg_uptime_pct: 100 };
      }
    },

    async getCameraStats() {
      return { total_triggers_24h: 0 };
    },
    ...
    async bulkOperation(cameraIds, operation, reason) {
      ...
      for (const id of cameraIds) {
        try {
          if (operation === 'enable') {
            await fetch(`/api/cameras/${id}/enable`, { method: 'POST' });
          } else if (operation === 'disable') {
            await fetch(`/api/cameras/${id}/disable`, { method: 'POST' });
    ...
    async resetCamera(cameraId) {
      await fetch(`/api/cameras/${cameraId}/disable`, { method: 'POST' });
      await fetch(`/api/cameras/${cameraId}/enable`, { method: 'POST' });
      return { status: 'reset' };
    },
  };
    ...
    return `
      <div class="camera-card ${statusClass} ${isSelected ? 'selected' : ''}"
        ...
        <div class="card-body" data-action="view-details">
          ...
          <p class="card-id" style="font-size:11px;color:#6b7a99;margin-top:2px;">ID: ${camera.camera_id}</p>
  ...
  function renderUptimeChart(snapshots) {
    ...
    const points = snapshots.slice(-20);
