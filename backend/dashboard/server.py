...
# Configure root logger so all backend.* loggers emit to stdout/stderr
logging.basicConfig(
    level=logging.INFO,
...
async def lifespan(app: FastAPI):
    ...
    # ── 5. Initialize PipelineV2 (YOLO gate + BoT-SORT + incident builder) ──
    try:
        ...
        pipeline_v2 = PipelineV2(
            ...
            redis_client=upstash_redis,
        )
        app.state.pipeline_v2 = pipeline_v2
        logger.info("PipelineV2 initialized")
    except Exception as e:
        logger.error("PipelineV2 initialization failed: %s", e)
        logger.warning("Running without PipelineV2 — falling back to existing pipeline")
        app.state.pipeline_v2 = None

    # ── 6. Start session-cleanup background task ──────────────
    # Closes out camera sessions that haven't seen motion for SESSION_TIMEOUT_SEC.
    try:
        triggers_api.start_session_cleanup_loop()
