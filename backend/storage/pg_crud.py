...
class PostgresCRUD:
    ...
    async def get_event_by_incident_id(self, incident_id: str, user_id: str) -> Optional[Event]:
        """Get an event by its incident_id (string ID like 'EVT_...')."""
        async with create_session() as session:
    ...
    async def update_event(
        ...
        async def _update(s: AsyncSession) -> Optional[Event]:
            ...
            if gemini_decision is not None:
                event.gemini_decision = gemini_decision
            if timeline_json is not None:
                event.timeline_json = timeline_json
