...
class GUID(TypeDecorator):
    ...
    Uses String(200) on all dialects to support both UUID and
    string-based IDs (e.g., 'dev-user-001').
    """
    ...
    def load_dialect_impl(self, dialect):
        return dialect.type_descriptor(String(200))
...
class ApiKey(Base):
    """API key for programmatic access to Vision OS services."""
    ...
    __table_args__ = (
        UniqueConstraint("key_prefix", "key_hash", name="uq_api_key_prefix_hash"),
    )
...
class ApiKeyUsage(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    key_id: Mapped[str] = mapped_column(String(32), ForeignKey("api_keys.id"), nullable=False)
    user_id: Mapped[str] = mapped_column(GUID(), nullable=False)
    timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("key_id", "timestamp", name="uq_key_timestamp"),
    )
...
class Payment(Base):
    """Payment transaction records for bKash/Nagad manual payments."""
    ...
    payment_method: Mapped[str] = mapped_column(String(20), default="bkash")
